...
def etusivu():
    if "user.id" in session:
        username = users.get_username(session["user.id"])
...
def ketju1():
    if "user_id" in session: 
        ...
        messages = messages.get_ketju_messages("ketju1")
...
def ketju2():
    if "user_id" in session: 
        username = users.get_username(session["user_id"])
        messages = messages.get_ketju_messages("ketju2")
        count_messages = len(messages)
...
def ketju3():
    if "user_id" in session: 
        username = users.get_username(session["user_id"])
        messages = messages.get_ketju_messages("ketju3")
        count_messages = len(messages)
